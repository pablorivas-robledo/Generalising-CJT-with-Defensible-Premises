...
pairwise_condorcet = function(voters, options, evidence){
...
pairwise_condorcet(total.voters, options, p_e)
...
while (max - calc > 0.00001) {
  calc = pairwise_condorcet(start , options, p_e)
  print(start)
  print(calc)
  gc()
  start = start + 1
...
p = c()
for (i in values){  
  p<- c(p, pairwise_condorcet(i , options, p_e) )
}
