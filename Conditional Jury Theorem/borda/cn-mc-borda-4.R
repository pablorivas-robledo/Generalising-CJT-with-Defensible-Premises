...
simulate_borda <- function(n_voters, probs, p_e) {
  results <- map_dbl(1:10000, ~simulate_jury(n_voters, probs))
  mn = mean(results)
  return((mn*p_e)+((1-mn)*(1-p_e)))
}
...
simulate_borda(n_voters, probs, p_e)

## asymptote
options =  c(0.4, (1-0.4)/3, (1-0.4)/3, (1-0.4)/3)
max = 0.51
calc = 0
...
while (max - 0.001 > calc) {
  calc = simulate_borda(start , probs, max)
  print(start)
  print(calc)
  gc()
  start = start + 1
